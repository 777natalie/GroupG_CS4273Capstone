...
def load_segments(txt: str) -> List[Dict[str, Any]]:
    segs = []
...
def grade_from_txt(txt: str) -> Dict[str, Dict[str,str]]:
    segs = load_segments(txt)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python test_case.py <transcript.json>")
        sys.exit(1)

    text = json_to_text(sys.argv[1])
    result = grade_from_txt(text)
